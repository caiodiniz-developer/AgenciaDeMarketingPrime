        ...
        >
          ...
          <g className="est-malha" data-est="malha">
            {PONTOS.map((p, i) => {
              ...
              return (
                <line key={p.id} x1={p.x} y1={p.y} x2={q.x} y2={q.y} data-est-malha />
              );
            })}
          </g>
          ...
          <g className="est-foco" data-est="foco">
            <circle cx={FOCO.x} cy={FOCO.y} r="96" fill="url(#est-foco)" data-est-halo />
          ...
          <g className="est-pontos">
            {PONTOS.map((p) => (
              <circle key={p.id} cx={p.x} cy={p.y} r="5" data-est-ponto data-id={p.id} />
            ))}
          </g>
        </svg>
        ...
        <ul className="estrategia__rotulos" aria-hidden="true">
          {PONTOS.map((p) => (
            <li
              ...
              key={p.id}
              style={{ left: `${(p.x / 900) * 100}%`, top: `${(p.y / 560) * 100}%` }}
            >
              {p.rotulo}
            </li>
          ))}
          <li
            ...
            style={{ left: `${(FOCO.x / 900) * 100}%`, top: `${(FOCO.y / 560) * 100}%` }}
          >
            Uma direção
          </li>
        </ul>
